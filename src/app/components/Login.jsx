...
const Login = () => {
  ...
  useEffect(() => {
    // Dalam mode development, hanya periksa apakah ada token development
    if (isDevelopment) {
      const turnstileData = sessionStorage.getItem('turnstileData');
      if (!turnstileData || !JSON.parse(turnstileData).token.includes('dev_mode')) {
        router.push('/');
      }
      return;
    }

    // Mode production - periksa token dan waktu verifikasi
    const turnstileData = sessionStorage.getItem('turnstileData');
      ...
      <form onSubmit={handleSubmit} className="bg-primary p-6 sm:p-8 md:p-9 w-full max-w-[700px] rounded-md mx-4">
        ...
        <div className="mt-6 text-center">
          ...
          <div className="mt-2 flex items-center justify-center gap-4">
            <a href="https://saweria.co/rendiichtiar" target="_blank" className="text-xs text-font flex items-center gap-1">
              Support me on 🎁<span className="underline">Saweria</span>
            </a>
          </div>
          <div className="mt-2 flex items-center justify-center">
            <a href="https://github.com/rendiichtiarp/prank-snbp-web" target="_blank" className="text-xs text-font flex items-center gap-1">
              ...
              <span className="underline">Github</span> - Jangan lupa kasih star ya!
            </a>
          </div>
        </div>
      </form>
